...
public class ServerFHIRValidation {
    ...
    public ValidationResult validateWithResult(IBaseResource resource, ValidationOptions validationOptions) {
        ...
        while(var11.hasNext()) {
            ...
            if (next11.getMessage().startsWith("Profile Reference")) continue;
            ...
            if (next11.getMessage().contains("Could not match discriminator ([system])")) continue;
            if (next11.getMessage().contains("Unable to find matching profile")) continue;
            // Work around of known faults.
            if (next11.getMessage().contains("units-of-time")
                    || next11.getMessage().contains("medicationdispense-type")
                    || next11.getMessage().contains("prescription-charge-exemption")) {
                next11.setSeverity(ResultSeverityEnum.WARNING);
            }
            if (next11.getSeverity().ordinal() >= ResultSeverityEnum.ERROR.ordinal()) {
                log.error("Validation Message {}",next11.getMessage());
                throw new UnprocessableEntityException(ctx, validationResult.toOperationOutcome());
            } else {
                if (next11.getSeverity().ordinal() >= ResultSeverityEnum.WARNING.ordinal()) {
                    log.warn("Validation Message {}",next11.getMessage());
                } else {
    ...
    private void setProfile(IBaseResource resource) {
        if (resource instanceof DomainResource) {
            // Remove profile information for validation
            ((DomainResource) resource).getMeta().setProfile(new ArrayList<>());
        }
