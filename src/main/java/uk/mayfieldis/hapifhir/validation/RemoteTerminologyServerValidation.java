...
public class RemoteTerminologyServerValidation extends BaseValidationSupport implements IValidationSupport {
    private String myBaseUrl;
    private List<Object> myClientInterceptors = new ArrayList();

    public RemoteTerminologyServerValidation(FhirContext theFhirContext) {
        ...
        if (StringUtils.isBlank(theCode)) {
            ...
            if (resultValues.size() >= 1 && !StringUtils.isBlank((CharSequence) resultValues.get(0))) {
                ...
                if (success) {
                    ...
                    if (displayValues.size() > 0) {
                        String display = (String) displayValues.get(0);
                        retVal.setMessage(display);
                        if (display.contains("is not known")) {
